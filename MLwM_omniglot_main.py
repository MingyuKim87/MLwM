...
def parse_args():
    ...
    common.add_argument('--img_size', default=28, type=int, help='image size')
    ...
    common.add_argument('--update_lr', default=0.4, type=float, help='learning rate for inner update')
...
def train(model, save_model_path, initializer=torch.nn.init.xavier_normal_):
    ...
    omniglot_training_set = meta_Omniglot_dataset(args.n_way, args.k_shot_support, args.k_shot_query, \
        args.training_data_path, args.img_size, types=args.datatypes)

    omniglot_valid_set = meta_Omniglot_dataset(args.n_way, args.k_shot_support, args.k_shot_query, \
        args.test_data_path, args.img_size, types=args.datatypes)

    # dataloader
    train_loader = DataLoader(omniglot_training_set, batch_size=args.task_size, shuffle=True)
    valid_loader = DataLoader(omniglot_valid_set, batch_size=1, shuffle=True)
    ...
    remove_temp_files_and_move_directory(save_model_path, "/home/mgyukim/workspaces/result_MLwM", args.model, \
        args.encoder_type, args.beta_kl, "Omniglot", args.datatypes)
...
if __name__ == '__main__':
    ...
    if args.model == "MLwM":
        ...
        ENCODER_CONFIG = set_config_encoder(ENCODER_CONFIG_OMNIGLOT, args.encoder_type, args.encoder_output_dim)
