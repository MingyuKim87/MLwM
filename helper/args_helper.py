...
def get_model_dir_path(args, **kwargs):
    '''
    ...
    if kwargs.get("model_save", None) is True:
        ...
        model_dir_path = os.path.join(model_root, args.model, args.dataset, args.datatypes,\
            current_datetime)
        ...
        # Spliting directory and file path and return directory path + "temp"
            # Usually, on test mode
        model_dir_path, _ = os.path.split(args.model_path)
        model_root_dir_path = os.path.join(model_dir_path, 'temp')
