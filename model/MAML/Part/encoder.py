...
from model.MAML.Part.layer import *
...
class Deterministic_Conv_Encoder(Abstract_Encoder):
    ...
    def __init__(self, config, padding=0, stride=2, dilation=1):
        ...
        #Input Dim
        self._img_size = config[0]
        self._input_channel = config[1]
        self._filter_sizes = config[3]

        self._kernel_size = config[4]
        self._padding = padding
        self._stride = stride
        self._dilation = dilation

        # FC layer
        self._hidden_img_size = self._conv2d_img_size()
...
class Conv_Reparameterization_Encoder(Abstract_Encoder):
    ...
    def __init__(self, config, padding=0, stride=2, dilation=1):
        ...
        # Embedding Layers
        self._layers_mu = nn.ModuleList([])
        self._layers_rho = nn.ModuleList([])

        for i in range(len(self._filter_sizes)):
            if i == 0 :
                self._layers_mu.append(nn.Conv2d(self._input_channel, self._filter_sizes[0], self._kernel_size, \
                    stride=self._stride, padding=self._padding, dilation=self._dilation, groups=1))
                self._layers_rho.append(nn.Conv2d(self._input_channel, self._filter_sizes[0], self._kernel_size, \
                    stride=self._stride, padding=self._padding, dilation=self._dilation, groups=1))
            else:
                self._layers_mu.append(nn.Conv2d(self._filter_sizes[i-1], self._filter_sizes[i], self._kernel_size, \
                    stride=self._stride, padding=self._padding, dilation=self._dilation, groups=1))
                self._layers_rho.append(nn.Conv2d(self._filter_sizes[i-1], self._filter_sizes[i], self._kernel_size, \
                    stride=self._stride, padding=self._padding, dilation=self._dilation, groups=1))
    ...
    def forward(self, inputs):
        ...
        # Conv Embedding (mu) for Meta learning
        for i, layer in enumerate(self._layers_mu):
            hidden_mu = F.relu(layer(hidden_mu))

        # Flatten (num_task * n_way * k_shot, num_channel, imgsize, imgsize) --> (num_task, n_way, k_shot, reminging_dim)
        hidden_mu = hidden_mu.view(num_task, n_way, k_shot, -1)
        
        hidden_mu = self._last_layer_mu(hidden_mu) #(num_task, n_way, k_shot, output_dim)

        # Conv Embedding (rho) for Meta learning
        for i, layer in enumerate(self._layers_rho):
            hidden_rho = F.relu(layer(hidden_rho))
        
        # Flatten (num_task * n_way * k_shot, num_channel, imgsize, imgsize) --> (num_task, n_way, k_shot, reminging_dim)
        hidden_rho = hidden_rho.view(num_task, n_way, k_shot, -1)
        hidden_rho = self._last_layer_rho(hidden_rho) #(num_task * num_points, output_dim)
        ...
        # Reshape hidden to a image type
        hidden_reparameterization = hidden_reparameterization.view(num_task, n_way, k_shot, 1, img_size, -1)
