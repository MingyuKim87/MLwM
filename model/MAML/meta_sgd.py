...
class MetaSGD(nn.Module):
    ...
    def __init__(self, config, update_lr, update_step, initializer=None, is_regression=False, is_image_feature=True):
        ...
        self.update_lr = nn.Parameter(torch.FloatTensor([update_lr]))
        ...
        for i in range(task_size):
            ...
            if not is_adaptation:
                if not self._is_regression:
                    pred_q = F.softmax(logits, dim=1).argmax(dim=1) #[n_way * k_shot_query, ]
                    pred_y = pred_q.view(n_way, k_shot_query) #[n_way, k_shot_query]
                    pred_y_list.append(pred_y)

                else:
                    pred_y_list.append(logits)
                ...
                with torch.no_grad():
                    ...
                    logits = self.net(x_query[i], task_parameter, bn_training=True)
                    if not self._is_regression:
